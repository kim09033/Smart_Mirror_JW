var FUNCTIONSERVICE = {
	...
	whoIsSmartMirror : function($scope) {
		...
		if(responsiveVoice.voiceSupport()) {
	          responsiveVoice.speak("저는 금오공대 컴퓨터공학과 창설1조가 만든거예요.","Korean Female");
        }
		$scope.focus = "Rain";
	},
	Rain : function($scope) {
		...
		if(responsiveVoice.voiceSupport()) {
	          responsiveVoice.speak("밖에 비가 오군요, 우산을 챙기세요.","Korean Female");
        }
		$scope.focus = "Rain1";
	},
	goSleep : function($scope){
        ...
        if(responsiveVoice.voiceSupport()) {
          responsiveVoice.speak("안녕히주무세요!","Korean Female");
        }
        $scope.focus = "sleep";
	},
	wake : function($scope) {
		...
		if(responsiveVoice.voiceSupport()) {
            responsiveVoice.speak("안녕하세요. FIT미러에요!","Korean Female");
          }
    	$scope.focus = "default";
	},
	...
	news: function($scope) {
		...
		if(responsiveVoice.voiceSupport()) {
            responsiveVoice.speak("실시간 뉴스입니다.","Korean Male");
          }
	...
	lightOn : function() {
		...
		var cmd_path = "/home/pi/smartmirror-web-application-master/smart-mirror/node_modules/onoff";
		/* 커맨드 명령*/
